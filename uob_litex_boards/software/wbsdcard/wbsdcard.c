...
{
	return dev->iobase[offset/4];
}

static inline void ocsdc_write(struct ocsdc *dev, int offset, uint32_t data)
{
	dev->iobase[offset/4] = data;
}

static uint32_t ocsdc_get_voltage(struct ocsdc * dev) {
	uint32_t v = ocsdc_read(dev, OCSDC_POWER_CONTROL);
	uint32_t voltage = 0;
	if (v >= 1650 && v <= 1950)
...
static void ocsdc_set_buswidth(struct ocsdc * dev, uint32_t width) {
	if (width == 4)
...
static void ocsdc_set_timeout(struct ocsdc * dev)
{
	...
	uint32_t timeout_reg = timeout_us * (clk_freq / 1e6);
...
{
    printf("Run the wbsdcard test function\n");
}
...
static DSTATUS sd_disk_initialize(BYTE drv) {
	if (drv) return STA_NOINIT;
	if (sdcardstatus)
		sdcardstatus = wbsdcard_init() ? 0 : STA_NOINIT;
	return sdcardstatus;
}

static DRESULT sd_disk_read(BYTE drv, BYTE *buf, LBA_t block, uint32_t count) {
